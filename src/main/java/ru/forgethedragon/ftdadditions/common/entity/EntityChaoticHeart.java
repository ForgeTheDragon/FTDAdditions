package ru.forgethedragon.ftdadditions.common.entity;
    ...
    public void onUpdate() {
        ...
        switch (opPhase) {
            ...
            case 2: { //Particle field accepting items
                if (coresConsumed == 8 || age > 1240) {
                    age = 1240;
                    opPhase = 3;
                    if (coresConsumed < 2) {
                        EntityHeart item = new EntityHeart(worldObj, posX, posY, posZ, new ItemStack(FTDItems.heartChaotic));
                        item.motionX = 0;
                        item.motionY = 1;
                        item.motionZ = 0;
                ...
                for (EntityItem item : items) {
                    ...
                    if (Utills.getDistanceAtoB(posX, posY + 0.5, posZ, item.posX, item.posY, item.posZ) < 1) {
                        if (coresConsumed == 8 || worldObj.isRemote) break;
                        if (stack.getItem() != FTDItems.coreCorruption) {
                            item.motionX = 1;
                            item.motionY = 6;
                            item.motionZ = 1;
                            continue;
                        }
                        int needed = 8 - coresConsumed;
                    ...
                    for (int x = (int) posX - 5; x <= (int) posX + 5; x++) {
                        for (int y = (int) posY - 5; y <= (int) posY + 5; y++) {
                            for (int z = (int) posZ - 5; z <= (int) posZ + 5; z++) {
                                if (worldObj.getBlock(x, y, z) instanceof DraconicBlock && worldObj.getBlockMetadata(x, y, z) == 0) {
            ...
            case 4: {
                ...
                for (MultiblockHelper.TileLocation tile : blocks) {
                    if (!worldObj.isRemote)
                        worldObj.setBlock(tile.getXCoord(), tile.getYCoord(), tile.getZCoord(), FTDBlocks.blockChaotic, 0, 2);
